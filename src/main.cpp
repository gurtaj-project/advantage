...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Advantage: starting difficulty is 1 / 2^12
...
{
    // Advantage: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 12000); // Advantage: 12k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Advantage: 24 hours
static const int64 nTargetSpacing = 1 * 60; // Advantage: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Advantage: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Advantage: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc8, 0xd6, 0xdf};// Advantage: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AdvantageMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AdvantageMiner(CWallet *pwallet)
{
    printf("AdvantageMiner started\n");
    ...
    try { loop {
        ...
        printf("Running AdvantageMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AdvantageMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AdvantageMiner, pwallet));
}
